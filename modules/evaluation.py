...
# Model yükleme fonksiyonunu güncelle
@st.cache_resource
def load_model_and_explainer():
    try:
        model_path = os.path.join('models', 'model.pkl')
        explainer_path = os.path.join('models', 'explainer.pkl')
        ...
        if not os.path.exists(explainer_path):
            st.error(f"Explainer file not found at: {explainer_path}")
            return None, None
...
# Veri yükleme fonksiyonunu güncelle
@st.cache_data
def load_and_prepare_data():
    try:
        data_path = os.path.join('data', 'customer_churn_records.csv')
...
if model is not None and explainer is not None and X is not None and y is not None:
    # Tahminler
    ...
    # Model ve veriyi yükle
    df = pd.read_csv(os.path.join("data", "customer_churn_records.csv"))
    X = df.drop(['Exited', 'RowNumber', 'CustomerId', 'Surname', 'Complain'], axis=1)
    y = df['Exited']
